...
model_results_single_fold <- function(fold_number) {
  ...
  null_c_index <- Cindex(object = Surv(df$time_to_event, df$Event), predicted = null_survival_prediction) # , t_star = 5)
  ehr_c_index <- Cindex(object = Surv(df$time_to_event, df$Event), predicted = ehr_survival_prediction) # , t_star = 5)

  null_brier_score <- Brier(object = Surv(df$time_to_event), predicted = null_survival_prediction, t_star = 5)
  ehr_brier_score <- Brier(object = Surv(df$time_to_event), predicted = ehr_survival_prediction, t_star = 5)
...
saveRDS(results, "../results/1k_snomed/test_fg_model_fg_snomed_1k_ukb_auc_cindex_brier.rds")
