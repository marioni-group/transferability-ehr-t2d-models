...
topic_grouping_cox_lasso_single_fold <- function(topic_model_fold_number, cox_lasso_fold_number) {
  ...
  topic_model_path <- "models/r11_condition/20240814_topic_model_all_folds/"
  save_path <- "models/r11_condition/20240814_topic_model_all_folds/cox_lasso_models/"
  ...
  condition_topics_unique <- readRDS(paste0(topic_model_path, "process_topic_model_result_200_topics_all_folds.rds"))$condition_topics_unique
  ...
  topic_model_post_processing_result <- readRDS(paste0(topic_model_path, "process_topic_model_result_200_topics_all_folds.rds"))
...
for (cl_i in 1:5) {
  ...
  print(paste0("Current topic model fold: ", 1))
  system.time({topic_grouping_cox_lasso_single_fold(topic_model_fold_number = 1,
...
aucs <- lapply(1:5, function(cl_i) {
    ...
    result <- readRDS(paste0("models/r11_condition/20231124/cox_lasso_models/result_cl_fold_", cl_i, "_tm_fold_1.rds"))
    result$aucs
