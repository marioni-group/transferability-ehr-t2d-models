...
model_results_single_fold <- function(fold_number) {
  ...
  list(aucs = aucs, c_indexes = c_indexes, briers = briers,
       lasso_coefs = lasso_coefs, missing_lasso_ceofs = missing_lasso_coefs) # , ehr_score = ehr_score)
}

results <- lapply(1:5, model_results_single_fold)

saveRDS(results, "/Cluster_Filespace/Marioni_Group/Yipeng/prediction-pipelines/ehr_t2d/results/r11/test_fg_model_fg_snomed_1k_results_with_auc_cindex_brier.rds")
