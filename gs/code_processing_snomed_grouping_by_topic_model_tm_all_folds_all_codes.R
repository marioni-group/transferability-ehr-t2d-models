...
generate_snomed_grouping_by_topic_table <- function(tm_fold_number, cl_fold_number) {
  snomed_to_topic_table <- read.csv(paste0("/Cluster_Filespace/Marioni_Group/Yipeng/prediction-pipelines/ehr_t2d/models/r11_model_files/topic_model_grouping_tm_all_folds/condition_topics_unique_fold_", tm_fold_number, ".csv"))

  snomed_to_topic_table$topic <- paste0("t", snomed_to_topic_table$topic)

  lasso_coefs <- read.csv(paste0("/Cluster_Filespace/Marioni_Group/Yipeng/prediction-pipelines/ehr_t2d/models/r11_model_files/topic_model_grouping_tm_all_folds/snomed_grouped_by_topic_lasso_coefs_result_cl_fold_", cl_fold_number, "_tm_fold_", tm_fold_number, ".csv"))
  ...
  for (variable in variables) {
    ...
    if (topic %in% colnames(df_topic)) {
      df_topic[, topic] <- df_topic[, topic] | df[, variable]
    }
  }
  ...
  df_topic <- cbind(df_topic, df[, c("id", "Age", "sex", "Event", "time_to_event")])
  saveRDS(df_topic, paste0("/Cluster_Filespace/Marioni_Group/Yipeng/prediction-pipelines/ehr_t2d/data/r11/gs_all_codes_presence_cox_table_snomed_grouped_by_topic_model_tm_all_folds_cl_fold_", cl_fold_number, "_tm_fold_", tm_fold_number, ".rds"))
}
